...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheapRec( int j ) {
	  ...
	  if (compare(heap.get(j), heap.get(parent(j))) < 1) {
		  swap(j, parent(j));
  ...
  protected void downheapRec( int j ) {
	  ...
	  if(hasLeft(j)){
		  ...
		  if(compare(heap.get(j), heap.get(min)) >= 0) {
				  swap(j, min);
			  }
  ...
  public int updateKey( K currentKey, K newKey ) {
	...
	// You may use loops in this method.
	  for(int k=1; k<heap.size(); k++) {
		  Entry<K,V> iterator = heap.get(k);
		  K key = iterator.getKey();
		  V value = iterator.getValue();
			
		  Entry<K,V> notEqual = new PQEntry<>(newKey, value);
			
		  if (currentKey.equals(key)) {
			  heap.set(k, notEqual);
			  heapify();
			  
			  return heap.indexOf(notEqual);
		  }
	  }
